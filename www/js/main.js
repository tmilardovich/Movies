...
function dohvacanje() {
  ...
  zahtjev.open(
    'GET',
    'http://api.themoviedb.org/3/search/movie?api_key=dd0f5e086bb2f44bd3a850f498e183c0&query=' +
      unos,
    true
  );
  zahtjev.send();
...
function dohvacanjeMostPopular() {
  ...
  zahtjev.open(
    'GET',
    'http://api.themoviedb.org/3/discover/movie?api_key=dd0f5e086bb2f44bd3a850f498e183c0&sort_by=popularity.desc',
    true
  );
  zahtjev.send();
}
function dohvacanjePoGodini() {
  ...
  var p =
    'https://api.themoviedb.org/3/discover/movie?api_key=dd0f5e086bb2f44bd3a850f498e183c0&sort_by=popularity.desc&include_adult=false&primary_release_year=';
  var d = unos;
  var rez = p + d;
  var objekt;
  ...
  zahtjev.onreadystatechange = function () {
    if (zahtjev.status == 200 && zahtjev.readyState == 4) {
      ...
      document.getElementById(
        'vijest'
      ).innerHTML = `10 najpopularnijih filmova iz ${unos}. godine su:`;
      for (let i = 0; i < 10; i++) {
        var putanjaSlike = objekt.results[i]['poster_path'];
        var fullPutanja = 'https://image.tmdb.org/t/p/w300' + putanjaSlike;
        var div = document.createElement('div');
